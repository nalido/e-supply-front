...
- 前端优先保证可复用组件和清晰结构。
- 新功能只要改变现有数据模型、字段语义或业务身份，就必须同步设计并验证旧数据迁移；能从原档案、审计快照或历史业务单据唯一确定的数据必须自动迁移，禁止让用户重复录入。无法唯一判断的数据应保留原值和业务引用，提供预填候选供用户确认，不得静默猜测、清空或改绑历史数量。迁移必须包含幂等、可追溯、回滚/备份考虑，以及迁移前后数据核对和相关接口、页面回归。
