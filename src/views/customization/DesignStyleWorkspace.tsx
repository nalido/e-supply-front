import { ArrowLeftOutlined, CheckCircleOutlined, CloudUploadOutlined, DownloadOutlined, LockOutlined, RedoOutlined, RobotOutlined } from '@ant-design/icons'
  ...
  return <div className="pod-page pod-ai-design-page">
    ...
    <Row gutter={[20, 20]}>
      ...
      <Col xs={24} xl={9}><Card className="pod-panel" title="整套商品效果图" extra={<Typography.Text type="secondary">批次 {style?.generationBatchNo?.slice(0, 8) || '尚未生成'}</Typography.Text>}>
        ...
        {style?.status === 'APPROVED' && selectedTemplate && <Alert className="pod-print-export-hint" type="info" showIcon message="印刷文件可以导出" description={`系统会按 ${printableAreaCount} 个正面、背面或侧面印区各自的成品尺寸和 ${selectedTemplate.minDpi} DPI，生成保留透明通道的 CMYK TIFF 文件包。`} />}
