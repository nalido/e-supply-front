...
type SupplierPaymentFormValues = Omit<SupplierPaymentPayload, 'date'> & { date?: Dayjs };

const SettlementSupplierPayments = () => {
  const [form] = Form.useForm<SupplierPaymentFormValues>();
  const [meta, setMeta] = useState<SupplierPaymentMeta | null>(null);
  const [metaLoading, setMetaLoading] = useState(false);

  const [records, setRecords] = useState<SupplierPaymentRecord[]>([]);
  const [summary, setSummary] = useState<SupplierPaymentListResponse['summary']>({
    totalPayable: 0,
    totalPaid: 0,
    totalArrears: 0,
  });
  ...
  const loadMeta = useCallback(async () => {
    setMetaLoading(true);
    try {
      const response = await supplierPaymentService.getMeta();
      setMeta(response);
    } catch (error) {
      console.error('failed to load supplier payment meta', error);
      message.error('加载供应商与账户配置失败');
    } finally {
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      const response = await supplierPaymentService.getList({
        page,
        pageSize,
        ...filters,
      });
      ...
      console.error('failed to load supplier payment list', error);
      message.error('获取供应商付款列表失败');
    } finally {
  ...
  const openPaymentModal = useCallback((supplierId?: string) => {
    const defaultDate = dayjs();
    setFixedSupplierId(supplierId);
    form.resetFields();
    form.setFieldsValue({
      supplierId,
      date: defaultDate,
  ...
  const handleExport = async () => {
    try {
      const result = await supplierPaymentService.export({
        page,
        pageSize,
        ...filters,
      });
      ...
      console.error('failed to export supplier payment report', error);
      message.error('导出失败，请稍后重试');
    }
  };

  const tableColumns: ColumnsType<SupplierPaymentRecord> = useMemo(() => [
    {
      title: '供应商',
...
export default SettlementSupplierPayments;
