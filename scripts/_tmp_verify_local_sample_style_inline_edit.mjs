...
const outDir = `/Users/huangjianbing/.openclaw/workspace/outbox/local-sample-style-inline-${Date.now()}`;
...
try {
  ...
  const helperTexts = await page.locator('text=当前显示的是关联款式').allInnerTexts().catch(() => []);
  ...
  result.helperTexts = helperTexts;

  await page.screenshot({ path: path.join(outDir, '02-edit-modal-top.png'), fullPage: true });
  await page.getByText('物料清单').first().scrollIntoViewIfNeeded();
  await sleep(1000);
  await page.screenshot({ path: path.join(outDir, '03-edit-modal-materials.png'), fullPage: true });
