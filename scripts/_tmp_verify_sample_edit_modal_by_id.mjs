...
const sampleId=6;
const outDir=`/Users/huangjianbing/.openclaw/workspace/outbox/sample-edit-modal-${Date.now()}`;
...
try {
 await login();
 await page.goto(`${baseUrl}/sample/detail?id=${sampleId}&mode=edit`, { waitUntil:'domcontentloaded', timeout:60000 });
 await sleep(4000);
 result.url=page.url();
 result.hasEditModal=await page.getByText('编辑样板单').count();
 result.hasSaveLinked=await page.getByRole('button', { name:'保存关联款式资料' }).count();
 result.hasSampleSave=await page.getByRole('button', { name:'保存' }).count();
 result.bodySnippet=(await page.locator('body').innerText()).slice(0,2000);
 await page.screenshot({ path:path.join(outDir,'01-sample-detail-edit.png'), fullPage:true });
 if (result.hasEditModal) {
   await page.getByText('物料清单').first().scrollIntoViewIfNeeded().catch(()=>{});
   await sleep(1000);
   await page.screenshot({ path:path.join(outDir,'02-sample-detail-edit-materials.png'), fullPage:true });
 }
